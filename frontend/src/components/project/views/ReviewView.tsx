...
export function ReviewView({ projectId, userId, onContinue, onUpdate }: ReviewViewProps) {
  ...
  const handleContinue = async () => {
    ...
    if (!approvedVideo) {
      setError('Please approve a video to continue');
      return;
    }
  ...
  const approvedVideo = videos.find(v => v.approved && v.status === 'completed');
  const canContinue = !!approvedVideo;
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Generate and review your final video output.
          </p>
        </div>
        ...
        <div className="pt-6 border-t border-gray-200">
          {!canContinue ? (
            <button
              ...
              title="Approve a video to continue"
            >
              Continue to Step 5
            </button>
          ) : (
          ...
          {!canContinue && videos.length > 0 && (
            <p className="mt-2 text-xs text-gray-500 text-center">
              Approve a video to continue
            </p>
          )}
        </div>
      </div>
